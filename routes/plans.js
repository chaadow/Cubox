...
//    connection.query('SELECT * FROM plans ORDER BY id DESC LIMIT ' + limit, function handleSql(err, rows) {
//        if (err){ logAndRespond(err,res); return; }
...
//    connection.query('UPDATE plans SET ? WHERE id='+req.params.id, req.query, function handleSql(err) {
//        if (err){ logAndRespond(err,res); return; }
//        handleFind(connection,req,res)
//    });
//}
//function handleDel(connection,req,res) {
//    connection.query('DELETE FROM plans WHERE id = ?', req.params.id, function handleSql(err) {
//        if (err){ logAndRespond(err,res); return; }
//        res.send({
//            result: 'success',
//            err:    '',
